"""Generate ``notebooks/01_dataset_exploration.ipynb`` — a phased, image-heavy tour of the dataset.

The notebook is *generated* (not hand-edited) so it can be regenerated and re-executed cleanly each
iteration; every phase maps to a phase in the approved plan, making progress easy to track.

Usage:
    python scripts/build_notebook.py            # write the .ipynb
    jupyter nbconvert --to notebook --execute --inplace notebooks/01_dataset_exploration.ipynb
"""
...
NB_PATH = Path(__file__).resolve().parents[1] / "notebooks" / "01_dataset_exploration.ipynb"
...
cells.append(md(
    ...
    "\n"
    "**Project:** automatic focal-liver-lesion **localization + classification + segmentation** from "
    "CEUS/RF ultrasound (Dresden → Halle external split).\n"
    "\n"
    "This notebook is *generated* by `scripts/build_notebook.py` and organized into **phases** that "
    "mirror the project plan, so each iteration's progress is easy to follow. Rerun the builder + "
    "`nbconvert --execute` to refresh it.\n"
    "\n"
    "| Phase | What it shows |\n"
    "|---|---|\n"
...
cells.append(md(
    ...
    "Every GUI-labeled case, cross-referenced with raw acquisitions, spline masks, and the clinical/QC "
    "spreadsheets. Site is derived from the case-ID prefix (`UKH…` = Halle, else Dresden)."
))
cells.append(code(
    "cohort = build_cohort()\n"
...
cells.append(md(
    ...
    "Pick one case and inspect its on-disk structure: DICOM cines, extracted RF/envelope acquisitions, "
    "and the acquisition geometry parsed from the YAML sidecars."
))
cells.append(code(
    "CASE = 'CEUS014'\n"
...
cells.append(md(
    ...
    "Three routes to a B-mode image: the **DICOM** display cine (already scan-converted), the "
    "**pre-scan envelope** (`*_env.raw`), and **RF → B-mode** via analytic-signal envelope + log "
    "compression. These are *pre-scan* (rectangular depth × line) for env/RF; scan-conversion comes "
...
cells.append(md(
    ...
    "The **critical open task**: GUI boxes, spline masks, and the DICOM display live in *different* "
    "coordinate systems. Here we print their coordinate ranges against each candidate image's "
    "dimensions and overlay them, to diagnose which space each label belongs to."
))
cells.append(code(
...
cells.append(md(
    ...
    "A representative DICOM frame per case for a few cases of each class — a quick visual sense of "
    "how FNH, hemangioma and metastasis look in this cohort."
))
cells.append(code(
    "def rep_frame(case):\n"
...
cells.append(md(
    "## Phase 6 — Next steps\n"
    "1. **Resolve label↔image registration** (Phase 4): fix the transform mapping GUI boxes & splines "
    "onto a single canonical B-mode; add a QA overlay gallery across sites/classes.\n"
    "2. **Scan-conversion** of the curvilinear geometry so pre-scan → true B-mode matches the DICOM.\n"
    "3. **Dataset export** (YOLO-seg + COCO) with boxes + masks + class, honoring the Dresden/Halle "
    "split and QC exclusions.\n"
